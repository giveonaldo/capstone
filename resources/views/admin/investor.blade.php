        ...
        <table class="w-full text-sm text-left rtl:text-right mt-10 mb-6 text-gray-500">
            <thead class="text-xs text-gray-700 uppercase bg-gray-50">
                <tr class="border-b">
                    ...
                    <th scope="col" class="px-6 py-3">valuasi</th>
            ...
            <tbody>
                @foreach ($investors as $investor)
                    <tr class="bg-white border-b">
                        ...
                        <td class="px-6 py-4">{{ $investor->valuasi }}</td>
                        <td class="px-6 py-4">{{ $investor->status }}</td>
                        <td class="px-6 py-4">
                            <form action="/admin/petani/mitra/{{ $investor->id }}" method="POST">
                                @csrf
                                @method('DELETE')
